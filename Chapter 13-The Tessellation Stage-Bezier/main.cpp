...
void BasicTessellation::DrawScene()
{
	...
	immediate_context_->IASetInputLayout(InputLayouts::Pos);
    immediate_context_->IASetPrimitiveTopology(D3D11_PRIMITIVE_TOPOLOGY_16_CONTROL_POINT_PATCHLIST);
    ...
    {
		immediate_context_->IASetVertexBuffers(0, 1, &mQuadPatchVB, &stride, &offset);
		...
		Effects::BezierTessellationFX->TessTech->GetPassByIndex(p)->Apply(0, immediate_context_);

		immediate_context_->RSSetState(RenderStates::WireframeRS);
		immediate_context_->Draw(16, 0);
	 }
...
void BasicTessellation::BuildQuadPatchBuffer()
{
    ...
    HR(device_->CreateBuffer(&vbd, &vinitData, &mQuadPatchVB));
}
